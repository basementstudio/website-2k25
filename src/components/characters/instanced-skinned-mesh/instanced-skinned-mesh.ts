...
import { configureCharacterMaterial } from "@/shaders/material-characters"
...
export class InstancedBatchedSkinnedMesh extends THREE.BatchedMesh {
  ...
  public dispose() {
    ...
    this.batchingKeyframeTexture.dispose()
    this.dataTextures.forEach((texture) => texture.dispose())
    this.programCompiledSubscribable.clearCallbacks()
    super.dispose()
    return this
  }

  public prepareMaterial() {
    ...
    if (!(this.material instanceof SiteMaterial))
      throw new Error("Character material must use nodes")
    this.material.uniforms.batchingKeyframeTexture = {
      value: this.batchingKeyframeTexture
    }
    this.material.uniforms.boneTexture = { value: this.boneTexture }
    if (this.useMorphs)
      this.material.uniforms.morphDataTexture = { value: this.morphTexture }
    this.dataTextures.forEach((texture, name) => {
      ;(this.material as SiteMaterial).uniforms[name] = { value: texture }
    })
    configureCharacterMaterial(
      this.material,
      (this as unknown as { _matricesTexture: THREE.DataTexture })
        ._matricesTexture
    )
  ...
  public update(delta: number): void {
    ...
    for (let i = 0; i < this.instanceData.length; i++) {
      ...
      const keyframe = offset + frame * skeleton.bones.length
      if (this.batchingKeyframeTexture.image.data![i] !== keyframe) {
        this.batchingKeyframeTexture.image.data![i] = keyframe
        changed = true
      }
    }
    if (changed) this.batchingKeyframeTexture.needsUpdate = true
  }
}
