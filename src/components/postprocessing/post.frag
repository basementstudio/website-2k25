...
vec3 dither(vec2 uv, vec3 color) {
    ...
    vec3 rightColor = texture2D(uMainTexture, uv + vec2(texelSize.x, 0.0)).rgb;
    vec3 bottomColor = texture2D(uMainTexture, uv + vec2(0.0, texelSize.y)).rgb;
    
    // Calculate color differences
    float rightDiff = length(color.rgb - rightColor);
    float bottomDiff = length(color.rgb - bottomColor);
    float maxDiff = max(rightDiff, bottomDiff);
    
    // Dither intensity based on differences and tolerance
    float ditherIntensity = smoothstep(0.0, uTolerance, maxDiff);
    
    // Apply dither 
    color.rgb += (threshold - 0.5) * ditherIntensity;
