...
void main() {
  ...
  if (vMapIndex < 0.5) {
    mapSample = sampleConfigMap(mapConfigs[0].map, mapConfigs[0].mapTransform);
  }
  #endif
  #if MULTI_MAP_COUNT >= 2
  if (vMapIndex >= 0.5 && vMapIndex < 1.5) {
    mapSample = sampleConfigMap(mapConfigs[1].map, mapConfigs[1].mapTransform);
  }
  #endif
  #if MULTI_MAP_COUNT >= 3
  if (vMapIndex >= 1.5 && vMapIndex < 2.5) {
    mapSample = sampleConfigMap(mapConfigs[2].map, mapConfigs[2].mapTransform);
  }
  #endif
  #if MULTI_MAP_COUNT >= 4
  if (vMapIndex >= 2.5 && vMapIndex < 3.5) {
    mapSample = sampleConfigMap(mapConfigs[3].map, mapConfigs[3].mapTransform);
  }
  #endif
