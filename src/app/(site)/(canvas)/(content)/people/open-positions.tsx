...
import type { OpenPositionItem } from "./sanity"

export const OpenPositions = ({ data }: { data: OpenPositionItem[] }) => (
  <section
  ...
  >
    ...
    <ul className="relative col-span-full text-brand-w2 lg:row-start-1">
      ...
      {data.map(({ title, slug, type, location, isOpen }, idx) => (
        <li key={idx} className="h- group relative grid grid-cols-12 gap-2">
          <Link
            href={isOpen ? `/careers/${slug}` : "#"}
            className={cn(
